...
function Navbar({ currentPage }) {
    ...
    return (
        <header className='w-full fixed z-20 top-0 bg-white border-b border-gray-200 flex justify-center'>
            ...
            {mobileMenuOpen && (
                <ul className='mx-4 p-2 flex flex-col gap-4 bg-gray-100 rounded fixed w-[calc(100%-2rem)] z-20 top-[84px] md:hidden'>
                    <li className={`uppercase rounded cursor-pointer ${currentPage === 'home' ? 'bg-primary-default text-white' : 'text-gray-950 hover:bg-gray-200'}`}>
                        <Link className='inline-block w-full p-3 text-end' to='/'>início</Link>
                    </li>
                    <li className={`uppercase rounded cursor-pointer ${currentPage === 'services' ? 'bg-primary-default text-white' : 'text-gray-950 hover:bg-gray-200'}`}>
                        <Link className='inline-block w-full p-3 text-end' to='/services'>serviços</Link>
                    </li>
                    <li className={`uppercase rounded cursor-pointer ${currentPage === 'about' ? 'bg-primary-default text-white' : 'text-gray-950 hover:bg-gray-200'}`}>
                        <Link className='inline-block w-full p-3 text-end' to='/about'>quem somos</Link>
                    </li>
                    <li className={`uppercase rounded cursor-pointer ${currentPage === 'contact' ? 'bg-primary-default text-white' : 'text-gray-950 hover:bg-gray-200'}`}>
