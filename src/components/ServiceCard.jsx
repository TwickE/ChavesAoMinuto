...
function ServiceCard({ service }) {
    ...
    const cardVisible = useScrollAnimation(cardRef, 220);

    return (
        <div ref={cardRef} className={`${cardVisible ? 'fadeInUp' : 'no-animation'} relative flex-col justify-center bg-gray-50 border-2 border-gray-100 hover:border-primary-default hover:bg-primary-superlight rounded-lg shadow-lg w-72 md:w-80`}>
            ...
            <Link to={`/services/${service.name}`} className="btn my-6 m-auto">Saber mais</Link>
        </div>
    )
}
