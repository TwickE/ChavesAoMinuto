...
function ServiceCard({ service }) {
    ...
    return (
        <div className="flex-col justify-center bg-gray-50 border-2 border-gray-100 hover:border-primary-default rounded-lg shadow-lg w-72 md:w-80">
            <img src={loadedImage} className='object-cover w-full max-h-44 rounded-lg' alt={service.imageDescription} />
            <h3 className='px-4 mt-4 text-lg text-primary-default text-center font-bold'>{service.name}</h3>
            <p className='px-4 mt-4 text-base text-justify text-gray-900'>{service.smallDescription}</p>
